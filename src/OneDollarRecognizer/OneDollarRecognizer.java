...
public class OneDollarRecognizer extends Preprocess {
    ...
    public Result Recognize(ArrayList<PointClass> points, boolean Protractor) {
        ...
        for (int i = 0; i < Unistrokes.size(); i++) {
            if(Protractor){
                d = OptimalCosineDistance(Unistrokes.get(i).Vector, candidate.Vector);
            }
            else {
                d = DistanceAtBestAngle(candidate.Points, Unistrokes.get(i), -AngleRange, +AngleRange, AnglePrecision);
            }
            System.out.println("Size of candidate points :" + candidate.Points.size() + "Size of Uni-stroke points"  + Unistrokes.get(i).Points.size());
            System.out.println("Checked with " + Unistrokes.get(i).Name + " and the distance is " + d);
            if (d < b) {
                b = d;
                u = i;
            }
        ...
        return u == -1 ? new Result("No match.", 0.0, timedif) : new Result(Unistrokes.get(u).Name, Protractor ? (1.0 - b) : score, timedif);
    }
