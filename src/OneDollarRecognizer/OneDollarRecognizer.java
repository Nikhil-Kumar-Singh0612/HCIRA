...
class Constants {
    static int NumUnistrokes = 16;
    static int NumPoints = 64;
    static double SquareSize = 250.0;
    static Point Origin = new Point(0, 0);
    static double Diagonal = Math.sqrt(SquareSize * SquareSize + SquareSize * SquareSize);
    static double HalfDiagonal = 0.5 * Diagonal;
...
class Helper extends Constants{
    ...
    public static double DistanceAtAngle(ArrayList<Point> points, Template T, double radians){
        ArrayList<Point> newPoints = RotateBy(points, radians);
        return PathDistance(newPoints, T.Points);
    }

    public static double DistanceAtBestAngle(ArrayList<Point> points, Template T, double a, double b, double threshold)
    {
    ...
    public ArrayList<Point> resample(ArrayList<Point> points, int n) {
        ...
        if (newpoints.size() == (n - 1)) {
            newpoints.add((n - 1), new Point(points.get(points.size() - 1).x, points.get(points.size() - 1).y));
        }
        return newpoints;
    }
    ...
    public static ArrayList<Point> RotateBy(ArrayList<Point> points, double radians) {
        ...
        for(int i=0; i<points.size(); i++) {
            ...
            Point p = new Point((int)qx,(int)qy);
            newpoints.add(p);
        }
        return newpoints;
    }

    public ArrayList<Point> ScaleTo(ArrayList<Point> points, double size) {
        Rectangle B = BoundingBox(points);
        ArrayList<Point> newpoints = new ArrayList<>();
        for( int i=0; i< points.size(); i++) {
            double qx = points.get(i).getX() *(size/B.Width);
            double qy = points.get(i).getY() *(size/B.Height);
            newpoints.add(new Point((int)qx,(int)qy));
        }
        return newpoints;
    }
    public ArrayList<Point> TranslateTo(ArrayList<Point> points, Point p){
        Point c = Centroid(points);
        ArrayList<Point> newpoints = new ArrayList<>();
        for (int i = 0; i < points.size(); i++) {
            double qx = points.get(i).getX() + p.getX() - c.getX();
            double qy = points.get(i).getY() + p.getY() - c.getY();
            newpoints.add(new Point((int)qx,(int)qy));
        }
        return newpoints;
    }
...
public class OneDollarRecognizer {
    ...
    public Result Recognize(ArrayList<Point> points){
        ...
        return (u == -1) ? new Result("No match.", 0.0, t1-t0) : new Result(Unistrokes[u].Name, (1.0 - b / Constants.HalfDiagonal), t1-t0);
    }

/*
    int AddGesture(String name, ArrayList<Point> points){
        Unistrokes[Unistrokes.length] = new Template(name, points);
        int num=0;
        for(int i=0;i<16; i++){
